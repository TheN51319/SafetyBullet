...
def evalute(model_dir,env,eval_episodes,save_dir):

    with open(os.path.join(model_dir, "config.json"), encoding="UTF-8") as f:
        config = json.load(f)
        seed = config.get('seed')

    eval_env, obs_space, act_space = make_sa_mujoco_env(num_envs=config['num_envs'], env_id="TheNMobileRobotBaseRadarEnv-v0", seed=None)
    ...
    models = os.listdir(model_dir+ '/torch_save')
    models = [model for model in models if model.endswith('.pt')]
    final_model_name = sorted(models)[-1]
    model_path = model_dir + '/torch_save' + '/' + final_model_name
    model.actor.load_state_dict(torch.load(model_path))
    ...
    #return sum(eval_rew_deque) / len(eval_rew_deque), sum(eval_cost_deque) / len(eval_cost_deque)
    print(f"reward:{sum(eval_rew_deque) / len(eval_rew_deque)}   costs:{sum(eval_cost_deque) / len(eval_cost_deque)}")

def evalute1(model_dir,eval_episodes):

    with open(os.path.join(model_dir, "config.json"), encoding="UTF-8") as f:
        config = json.load(f)
        seed = config.get('seed')

    eval_env, obs_space, act_space = make_sa_mujoco_env(num_envs=1, env_id="SafetyTheNMobileRobotBaseRadarEnv-v1", seed=None)
